...
function checkAppCache() {
	...
	$(document).ready(function() {	
		...
		// 建立 WebSocket 連接到後端，路徑為 ws://localhost:8080/ws
		let socket = io.connect('http://localhost:8080/ws');
		...
		socket.onmessage = function(event) {
			console.log("收到來自後端的回應:", event.data);
		};
		...
		document.addEventListener('mousemove', (event) => {
			...
			const threshold = 5;//設定最小移動間值
			if(Math.abs(deltaX) > threshold || Math.abs(deltaY) > threshold){
				//根據滑鼠移動的方向設定小精靈的方向
				if(Math.abs(deltaX) > Math.abs(deltaY)){
					if(deltaX > 0){
						pacman.directionWatcher.set(right);//向右
						direction = 'right';
					}else{
						pacman.directionWatcher.set(left);//向左
						direction = 'left';
					}
				}else{
					if(deltaY > 0){
						pacman.directionWatcher.set(down);//向下
						direction = 'down';
					}else{
						pacman.directionWatcher.set(up);//向上
						direction = 'up';
					}
				}
			}	
			...
			// 確保 WebSocket 連接已開啟
			if (socket.connected) {  // 使用 socket.connected 來檢查是否已連接
				socket.emit('message', coordinates); // 傳送資料到後端
				console.log("座標已傳送:", coordinates);
			} else {
			...
			socket.on('message', function(data) {
				console.log("收到來自後端的回應:", data);
				if (data.status === "success") {
					console.log("資料成功儲存", data.received_data);
				} else {
					console.error("資料儲存失敗");
				}
			});
